...
void moveObjects(vector < Figure > &figures, int del, int &drawCount, int &i);
...
int main()
{
   ...
   int numFigs; // number of boxes on screen
   int index = 0;
   while (drawCnt > 0)
   {
      ...
      // determines number of boxes on the screen 
      numFigs = getInt("Number of objects on screen (0 to exit): ", 15,
                       "Invalid Entry - must be a between 0 - 15!\n");
      ...
      for (int i = 0; i < numFigs; i++)
      {
         ...
         // store the object instance in the vector
         figureVec.push_back(rectangle);
         // change row/col so that each object has a different starting pos.
         col+= 5;
         row++;
      }
      
      // Recursive call. Move objects to next position
      moveObjects(figureVec, delay, drawCnt, index);
...
void moveObjects(vector < Figure > &figures, int del, int &drawCount, int &i)
{
   ...
   //Reached the end of vector and drawn a 'frame' of animation
   //Start back to beginning of vector and let recursion re-assign variables
   // and draw new frame
   if (i == figures.size())
   {
      i = 0;
   ...
   moveObjects(figures, del, drawCount, i);
